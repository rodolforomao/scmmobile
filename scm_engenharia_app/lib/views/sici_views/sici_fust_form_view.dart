...
import 'package:scm_engenharia_app/data/quantitative_distribution_physical_accesses_service_model.dart';
import 'package:scm_engenharia_app/data/sici_file_model.dart';
import 'dart:async';
...
class SiciFustFormView extends StatefulWidget {
  ...
  SiciFustFormView({ Key? key, required this.FichaSiciModel})
      : super(key: key);
...
class SiciFustFormState extends State<SiciFustFormView> {
  ...
  Future<Null> OnSalvarFormularioDbLocal() async {
    try {
      ...
      siciFileModel.periodoReferencia = _TxtControllerPeriodoReferencia.text;
      siciFileModel.isSincronizar = "S";
      siciFileModel.cnpj = _TxtControllerCnpj.text;
      if (_TxtControllerRazaoSocial.text.isEmpty)
        throw ("O campo Razão Social é obrigatório");
      siciFileModel.razaoSocial = _TxtControllerRazaoSocial.text;
      ...
      siciFileModel.telefoneMovel = _TxtControllerTelefoneMovel.text;
      siciFileModel.telefoneFixo = _TxtControllerTelefoneFixo.text;
      siciFileModel.receitaBruta = _TxtControllerReceitaBruta.text;
      siciFileModel.receitaLiquida = _TxtControllerReceitaLiquida.text;
      siciFileModel.simples = _TxtControllerSimples.text;
      siciFileModel.simplesPorc = _TxtControllerSimplesPorc.text;
      siciFileModel.icms = _TxtControllerIcms.text;
      siciFileModel.icmsPorc = _TxtControllerIcmsPorc.text;
      siciFileModel.pis = _TxtControllerPis.text;
      siciFileModel.pisPorc = _TxtControllerPisPorc.text;
      siciFileModel.cofins = _TxtControllerCofins.text;
      siciFileModel.cofinsPorc = _TxtControllerCofinsPorc.text;
      siciFileModel.observacoes = _TxtControllerObservacoes.text;
      if (siciFileModel.distribuicaoFisicosServicoQuantitativo!.length == 0)
        ...
        Operacao _respLocal = await dbHelper.OnAddFichaSici(siciFileModel);
        if (_respLocal.erro)
  ...
  Inc() async {
    try {
      ...
      if (widget.FichaSiciModel != null) {
        siciFileModel = widget.FichaSiciModel!;
